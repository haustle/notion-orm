...
import { DATABASES_DIR } from "../shared/constants";
import {
  AST_FS_PATHS,
  AST_FS_FILENAMES,
  AST_IMPORT_PATHS,
...
export function readAgentMetadataFromDisk(): CachedAgentMetadata[] {
  try {
    const agentsDir = path.join(process.cwd(), "build", "agents");
    const metadataFile = path.resolve(agentsDir, "metadata.json");
    if (!fs.existsSync(metadataFile)) {
      return [];
    }
    const content = fs.readFileSync(metadataFile, "utf-8");
...
/**
 * Main NotionORM class that provides access to all generated database and agent types
 */
