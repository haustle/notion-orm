...
async function runAdd(input: string): Promise<void> {
  ...
  console.log(
    `🔍 Found config: ${
      configFile.isTS ? "notion.config.ts" : "notion.config.js"
    }`
  );
  ...
  if (config.databaseIds.includes(databaseId)) {
    console.log(
      `⚠️  Database ID ${databaseId} is already in the configuration`
    );
  ...
  // Clear the cached config so that runGenerate() loads the updated config
  clearConfigCache();
