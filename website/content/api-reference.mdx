...
Optional **`NOTION_BASE_URL`** override for **`@notionhq/client`** (database / data-source APIs and matching CLI usage): set it to the REST **origin only**, e.g. `https://api.notion.com`, without `/v1`. **`@notionhq/agents-client`** does not use this variable in this package.
