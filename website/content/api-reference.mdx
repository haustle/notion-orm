...
| `create({ properties, icon?, cover? })` | Creates a page with typed properties |
...
Typed query filters now support most first-class Notion property filters. These property types are not supported in filters yet:
...
These property types are supported on your schema and in query results, but typed filter operators are not available for them:

| Property type | Why |
| ------------- | --- |
| `formula`     | Formula results can vary by expression and return type, so they are currently excluded from typed filter operators. |
