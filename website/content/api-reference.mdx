...
For **`@notionhq/client`** (database / data-source APIs, including matching CLI usage), optionally set **`NOTION_BASE_URL`** to the REST **origin only** (`https://api.notion.com`, without `/v1`). If unset or blank, **`NOTION_DEFAULT_BASE_URL`** applies. **`@notionhq/agents-client`** is not wired to this var in this package.
