...
For **`@notionhq/client`** (database / data-source APIs, including matching CLI usage), optionally set **`NOTION_BASE_URL`** to the REST **origin only** (`https://api.notion.com`, without `/v1`). **`NOTION_API_BASE_URL`** is a legacy fallback when the primary env is unset. **`@notionhq/agents-client`** is not wired to these vars in this package.
...
| `./notion/databases/<Database>` | `<Database>(auth)` factory, `PageSchema`, `CreateSchema`, `QuerySchema`, `columns` metadata, option tuples for select/status/multi-select, related type aliases | Direct database usage without the `NotionORM` wrapper |
