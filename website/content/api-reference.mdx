...
`bun notion sync` writes generated modules under **`notion/`** at your **project root** (next to `notion.config.ts`) — it is your app’s synced copy of available databases and agents.

**On-disk layout**

```txt
notion/
├── index.ts              # named export: NotionORM (plus re-exports from the ORM base module)
├── databases/
│   ├── index.ts          # `databases` registry (all DB factories)
│   ├── <Database>.ts     # one file per tracked database (PascalCase stem, e.g. CustomerOrders.ts)
│   └── metadata.json
└── agents/
    ├── index.ts          # `agents` registry (all agent factories)
    ├── <Agent>.ts        # one file per agent (PascalCase stem, e.g. MealAgent.ts)
    └── metadata.json
```

Registry keys on `notion.databases` / `notion.agents` use **camelCase** (from config). Per-module **file names** and **factory exports** use **PascalCase** (e.g. import `MealAgent` from `./notion/agents/MealAgent` while `notion.agents.mealAgent` is the runtime map entry).

**Imports** — prefer **project-relative** paths into `./notion/`. The same path segments work if your tooling maps `@haustle/notion-orm/notion/*` → `./notion/*` (the npm package does not ship your generated files; they only exist under `./notion/` after sync).
