...
Optional **REST host**: set **`NOTION_BASE_URL`** to the origin only (`https://api.notion.com`, no `/v1`). If omitted or blank, the package uses the **`NOTION_DEFAULT_BASE_URL`** origin (`https://api.notion.com`). This affects database / data-source APIs; `@notionhq/agents-client` is not wired to this env in this package. CLI database steps use `@notionhq/client` and follow the same resolution.
