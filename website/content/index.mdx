...
`notion sync` writes **generated modules under `notion/`** at your **project root** (next to `notion.config.ts`)—your synced database schemas, registries, and agent factories. A full sync replaces the entire `notion/` tree so removed databases or agents do not linger.

**Typical layout** (each `.ts` file has a matching transpiled `.js`; the root also emits declaration files for typed imports):
