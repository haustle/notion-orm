...
**What this does:**
1. Clones the SDK repository into a local cache (`node_modules/.cache/.notion-agents-sdk`) and adds the built `@notionhq/agents-client` package to your project

Run both commands to install the SDK and generate agent types. Agents linked to your integration are automatically discovered. When the upstream SDK changes, rerun `bun notion setup-agents-sdk` (it refreshes the cached clone, rebuilds, and reinstalls), then `bun notion sync`.
